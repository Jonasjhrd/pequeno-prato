...
export default function SignUpPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      console.log("[v0] Resposta do Google OAuth:", { data, error })

      if (error) {
        console.error("[v0] Erro no Google OAuth:", error)
        setGoogleError(`Erro: ${error.message}`)
      }
    } catch (error) {
      console.error("[v0] Erro ao conectar com Google:", error)
          ...
          <Card className="border-0 shadow-xl rounded-3xl bg-white/80 backdrop-blur-sm">
            <CardHeader className="space-y-2 pb-6">
              ...
              <CardDescription className="text-center text-base">
                Cadastre-se para acessar todas as receitas
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="relative mb-6">
                ...
                <div className="relative flex justify-center text-sm uppercase">
                  <span className="bg-white px-3 text-gray-500 font-medium">Ou cadastre-se com email</span>
                </div>
              </div>
